...
public class GUI_Client extends JFrame {
	...
	private JButton B_sync = new JButton("同步");
	...
	public GUI_Client() {
		...
		TA_border_title = new TitledBorder(UIManager.getBorder("TitledBorder.border"), "同步信息",
		...
		B_sync.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (ip.equals("") || port == 90000) {
					updateText("配置文件未找到，请求详细信息中");
					
					if (ip.equals("")) {
						ip = JOptionPane.showInputDialog("服务器IP地址");
						setIPAddress(ip);
					}
					
					if (port == 90000) {
						String serverPort = JOptionPane.showInputDialog("服务器端口(纯数字)");
	...
	private void internationalize() {
		setTitle(Main.strings.getString("title") + " - " + RefStrings.VERSION + " - Hao_cen魔改版");
	...
	public int getPort() {
		...
		try {
			...
			if (!(port <= 49151 && port > 0)) {
				updateText("端口超出范围！可选范围（ 1 - 49151）");
			}
